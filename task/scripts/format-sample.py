...
def main():
    ...
    for i, row in flights_df.iterrows():
      gate_id = row['gate']
      flights_df.at[i, 'gate'] = location_id_to_point_id_dict[gate_id]
      ...
      G.add_edge(flights_df.at[i, 'point_id'], location_id_to_point_id_dict[gate_id], dist=1500)
    ...
    header = ['location_id', 'point_id']
    with open(out_dirpath + '/points.csv', 'w') as f:
        f.write(','.join(header) + '\n')
        for location_id, point_id in location_id_to_point_id_dict.items():
            f.write(f'{location_id},{point_id}\n')
    ...
    # # Save graph
    # plt.savefig(out_dirpath + '/graph.png')
